...
// SEARCH RESULT STREAMS FOR MOVIES - THESE WILL PLAY THE VIDEO WHEN CLICKED
const getSearchMovieStreams = (tmdbId: string) => {
  return {
    "Server 1": `https://xprime.today/watch/${tmdbId}`,
    "Server 2": `https://cinemaos.tech/player/${tmdbId}`,
    "Server 3": `https://www.vidking.net/embed/movie/${tmdbId}?autoPlay=true&nextEpisode=true&episodeSelector=true`,
    "Server 4": `https://vidsrc-embed.ru/embed/movie/${tmdbId}`,
    "Server 5": `https://api.cinezo.net/embed/tmdb-movie-${tmdbId}`,
  };
};

// SEARCH RESULT STREAMS FOR TV SHOWS - THESE WILL PLAY THE VIDEO WHEN CLICKED
const getSearchTVStreams = (tmdbId: string) => {
  return {
    "Server 1": `https://www.vidking.net/embed/tv/${tmdbId}/${season}/${episode}?autoPlay=true&nextEpisode=true&episodeSelector=true`,
    "Server 2": `https://xprime.today/watch/${tmdbId}/${season}/${episode}`,
...
const getLocalMovieStreams = (tmdbId: string) => {
  return {
    "Server 1": `https://xprime.today/watch/${tmdbId}`,
    "Server 2": `https://cinemaos.tech/player/${tmdbId}`,
    "Server 3": `https://www.vidking.net/embed/movie/${tmdbId}?autoPlay=true&nextEpisode=true&episodeSelector=true`,
...
const getLocalTVStreams = (tmdbId: string, season: number = 1, episode: number = 1) => {
  return {
    "Server 1": `https://www.vidking.net/embed/tv/${tmdbId}/${season}/${episode}?autoPlay=true&nextEpisode=true&episodeSelector=true`,
    "Server 2": `https://xprime.today/watch/${tmdbId}/${season}/${episode}`,
    "Server 3": `https://api.cinezo.net/embed/tmdb-tv-${tmdbId}/${season}/${episode}`,
...
// GET ALL LOCAL ITEMS
const getAllLocalItems = (): MediaItem[] => {
  return [
    ...UNIQUE_MOVIES,
    ...UNIQUE_TV_SHOWS,
    ...UNIQUE_SPORTS,
...
// SEARCH FUNCTION WITH PROPER STREAMING LINKS THAT PLAY VIDEOS
export const searchContent = async (query: string): Promise<MediaItem[]> => {
  ...
  try {
    // FIRST: Search in ALL your local data
    const allLocalItems = getAllLocalItems();
    
    const localResults = allLocalItems.filter(item => {
      const titleMatch = item.title.toLowerCase().includes(searchTerm);
      const genreMatch = item.genres?.some(g => g.toLowerCase().includes(searchTerm));
    ...
    // SECOND: Try TMDB API for additional content
    ...
    if (response.ok) {
      ...
      if (data.results && data.results.length > 0) {
        const tmdbResults = data.results
          ...
          .map((item: any) => {
            // Check if we already have this in local results
            const existingLocal = allLocalItems.find(local => 
              local.id.includes(item.id.toString()) || 
              local.title.toLowerCase() === (item.title || item.name || '').toLowerCase()
            );
            ...
            // GET STREAMS FOR SEARCH RESULTS - THESE LINKS WILL PLAY THE VIDEO WHEN CLICKED
            let streams = {};
            ...
            return {
              id: `tmdb-${item.id}-${item.media_type}`,
              ...
              genres: item.genre_ids ? item.genre_ids.map((id: number) => `Genre ${id}`).slice(0, 3) : ['General'],
              streams: streams, // THESE ARE THE STREAMS THAT WILL PLAY THE VIDEO
    ...
    // Return local results even if TMDB fails
    const allLocalItems = getAllLocalItems();
    const localResults = allLocalItems.filter(item => {
      const titleMatch = item.title.toLowerCase().includes(searchTerm);
      const genreMatch = item.genres?.some(g => g.toLowerCase().includes(searchTerm));
...
export const fetchById = async (id: string, type: string): Promise<MediaItem | null> => {
  ...
  const localItem = source.find(i => i.id === id);
  ...
  if (id.startsWith('tmdb-')) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // USE SEARCH RESULT STREAMS FOR TMDB ITEMS - THESE WILL PLAY THE VIDEO
        let streams = {};
        if (mediaType === 'movie') {
        ...
        return {
          ...
          streams: streams, // THESE STREAMS WILL PLAY THE VIDEO
...
export const getAllContent = (): MediaItem[] => {
  return getAllLocalItems();
};

// GET ITEM BY ID AND TYPE
export const getContentByIdAndType = (id: string, type: string): MediaItem | null => {
